...
for m in range(10, 273, 12):
    if int(time_shift(shift(sheet_per.cell(row=m, column=1).value)[0])[0]) <= int(time_shift(time_now)[0]) \
            < int(time_shift(shift(sheet_per.cell(row=m, column=1).value)[1])[0]):
        h_i = m
        # print(h_i)
        break
for string in range(h_i, h_i + 12):
    if int(time_shift(sheet_per.cell(row=string, column=2).value)[0]) <= int(time_shift(time_now)[1]) \
            < int(time_shift(sheet_per.cell(row=string, column=2).value)[1]):
        m_i = string
        # print(m_i)
        break
...
while True:
    ...
    if not (int(time_shift(sheet_per.cell(row=m_i, column=2).value)[0]) <= int(time_shift(now_minus_6)[1])
            < int(time_shift(sheet_per.cell(row=m_i, column=2).value)[1])):
        print("----------------------------------")
